...
def add_transaction():
    ...
    json_out=out.json()
    if(status_code!=200):
        return jsonify({'message' : json_out['message']}),500
    transactions=json_out["SK_Fetch from F4211"]["rowset"]
    transaction_count=0
    for trans in transactions:
        transaction_count=transaction_count+1
        json=trans
        transaction_keys=['sender','receiver','itemcode','LOT','quantity']
        if not all (key in json for key in transaction_keys):
            return 'Some elements of the transation are missing for transaction number'+str(transaction_count),400
        index,message=distributor.add_transaction(blockchain,json['sender'],json['receiver'],json['itemcode'],json['LOT'],json['quantity'])
        if index==-1:
            return jsonify({'message' : str(message)+' for transaction number: '+str(transaction_count)}),400
